...
public class PayServiceImpl implements PayService {
    ...
    public PayConfigResponse getPayConfig() {
        ...
        response.setCreditPayStatus(Constants.CONFIG_FORM_SWITCH_OPEN.equals(creditPayStatus));
        ...
        // 设置授信支付配置 - 从系统配置中读取
        boolean isCreditPayOpen = Constants.CONFIG_FORM_SWITCH_OPEN.equals(creditPayStatus);
        
        // 获取用户授信信息 - 直接从用户表获取
        try {
            // 检查用户是否有有效的授信额度
            BigDecimal availableCreditAmount = user.getRemainingCredit() != null ? user.getRemainingCredit() : BigDecimal.ZERO;
            BigDecimal usedCreditAmount = user.getUsedCredit() != null ? user.getUsedCredit() : BigDecimal.ZERO;
            BigDecimal totalPendingAmount = user.getPendingRepayment() != null ? user.getPendingRepayment() : BigDecimal.ZERO;
            
            // 检查授信状态，只有已授信状态且系统开启授信支付才能使用
            boolean canUseCredit = user.getCreditStatus() != null && user.getCreditStatus() == 1 && isCreditPayOpen;
            
            response.setAvailableCreditAmount(canUseCredit ? availableCreditAmount : BigDecimal.ZERO);
            response.setUsedCreditAmount(usedCreditAmount);
            response.setTotalPendingRepaymentAmount(totalPendingAmount);
            response.setCreditPayStatus(canUseCredit);
        } catch (Exception e) {
            // 如果获取授信信息失败，设置为0
            response.setAvailableCreditAmount(BigDecimal.ZERO);
            response.setUsedCreditAmount(BigDecimal.ZERO);
            response.setTotalPendingRepaymentAmount(BigDecimal.ZERO);
            response.setCreditPayStatus(Boolean.FALSE);
        }
    ...
    public OrderPayResultResponse payment(OrderPayRequest orderPayRequest) {
        ...
        if (orderPayRequest.getPayType().equals(PayConstants.PAY_TYPE_CREDIT)) {
            ...
            if (!Constants.CONFIG_FORM_SWITCH_OPEN.equals(creditPayStatus)) {
                throw new CrmebException(CommonResultCode.VALIDATE_FAILED, "系统暂未开启授信支付功能");
            }
            
            // 检查用户授信状态
            if (user.getCreditStatus() == null || user.getCreditStatus() != 1) {
                throw new CrmebException(CommonResultCode.VALIDATE_FAILED, "用户未获得授信或授信状态异常");
            }
            
            // 检查剩余授信额度
            BigDecimal remainingCredit = user.getRemainingCredit() != null ? user.getRemainingCredit() : BigDecimal.ZERO;
            if (remainingCredit.compareTo(order.getPayPrice()) < 0) {
                throw new CrmebException(CommonResultCode.VALIDATE_FAILED, "可用授信额度不足");
            }
        }
        ...
        if (order.getPayType().equals(PayConstants.PAY_TYPE_CREDIT)) {
            Boolean creditBoolean = creditPay(order, user);
            response.setStatus(creditBoolean);
            return response;
        }
...
//        integralRecord.setBalance(balance);
    ...
    private Boolean creditPay(Order order, User user) {
        try {
            Boolean execute = transactionTemplate.execute(e -> {
                ...
                // 检查额度是否足够
                if (currentRemainingCredit.compareTo(payAmount) < 0) {
                    logger.error("授信支付，用户授信额度不足，orderNo = {}, remainingCredit = {}, payAmount = {}", 
                        order.getOrderNo(), currentRemainingCredit, payAmount);
                    e.setRollbackOnly();
                    return Boolean.FALSE;
                }
                
                // 更新用户授信额度
                user.setRemainingCredit(currentRemainingCredit.subtract(payAmount));
                user.setUsedCredit(currentUsedCredit.add(payAmount));
                user.setPendingRepayment(currentPendingRepayment.add(payAmount));
                
                Boolean userUpdateResult = userService.updateById(user);
                if (!userUpdateResult) {
                    logger.error("授信支付，更新用户授信额度失败，orderNo = {}", order.getOrderNo());
                    e.setRollbackOnly();
                    return Boolean.FALSE;
                }
                
                // 2. 创建授信支付订单（待还款记录）
                CreditPaymentOrder creditPaymentOrder = creditPaymentOrderService.createCreditPaymentOrder(
                    user.getId(), order.getOrderNo(), payAmount);
                
                if (ObjectUtil.isNull(creditPaymentOrder)) {
                    logger.error("授信支付，创建授信支付订单失败，orderNo = {}", order.getOrderNo());
                    e.setRollbackOnly();
                    return Boolean.FALSE;
                }
                
                // 2.1 修正用户待还款金额 - 应该是总还款金额（本金+利息），而不是只有本金
                BigDecimal totalRepaymentAmount = creditPaymentOrder.getTotalRepaymentAmount();
                BigDecimal interestAmount = totalRepaymentAmount.subtract(payAmount);
                user.setPendingRepayment(currentPendingRepayment.add(totalRepaymentAmount)); // 修正为总还款金额
                
                // 重新更新用户信息
                Boolean userUpdateResult2 = userService.updateById(user);
                if (!userUpdateResult2) {
                    logger.error("授信支付，修正用户待还款金额失败，orderNo = {}", order.getOrderNo());
                    e.setRollbackOnly();
                    return Boolean.FALSE;
                }
                ...
                order.setPayTime(DateUtil.date());
                order.setStatus(OrderConstants.ORDER_STATUS_WAIT_SHIPPING);
                order.setPayType(PayConstants.PAY_TYPE_CREDIT);
                order.setPayChannel(PayConstants.PAY_CHANNEL_CREDIT);
                order.setOutTradeNo(creditPaymentOrder.getCreditOrderNo());
                orderService.updateById(order);
                
                // 4. 生成授信支付交易记录
                Boolean transactionResult = creditTransactionService.createConsumptionTransaction(
                    user.getId(), order.getOrderNo(), payAmount);
                if (!transactionResult) {
                    logger.error("授信支付，创建交易记录失败，orderNo = {}", order.getOrderNo());
                    e.setRollbackOnly();
                    return Boolean.FALSE;
                }
            ...
            if (!execute) {
                throw new CrmebException("授信支付订单失败");
            }
            
            asyncService.orderPaySuccessSplit(order.getOrderNo());
            return Boolean.TRUE;
        } catch (Exception e) {
            logger.error("授信支付异常，orderNo = {}", order.getOrderNo(), e);
            throw new CrmebException("授信支付失败：" + e.getMessage());
        }
    }
     ...
     * @return Boolean
     */
    public Boolean creditRefund(String orderNo, BigDecimal refundAmount, Integer userId) {
        try {
            Boolean execute = transactionTemplate.execute(e -> {
                // 1. 获取用户信息
                User user = userService.getById(userId);
                if (ObjectUtil.isNull(user)) {
                    logger.error("授信退款，用户不存在，orderNo = {}, userId = {}", orderNo, userId);
                    e.setRollbackOnly();
                    return Boolean.FALSE;
                }
                
                // 2. 查找对应的授信支付订单
                CreditPaymentOrder creditPaymentOrder = creditPaymentOrderService.getByOriginalOrderNo(orderNo);
                if (ObjectUtil.isNull(creditPaymentOrder)) {
                    logger.error("授信退款，未找到对应的授信支付订单，orderNo = {}", orderNo);
                    e.setRollbackOnly();
                    return Boolean.FALSE;
                }
                ...
                // 计算需要减少的待还款金额（应该按比例减少总还款金额）
                BigDecimal refundRatio = refundAmount.divide(creditPaymentOrder.getPaymentAmount(), 6, RoundingMode.HALF_UP);
                BigDecimal totalRepaymentReduction = creditPaymentOrder.getTotalRepaymentAmount().multiply(refundRatio);
                
                // 恢复授信额度
                user.setRemainingCredit(currentRemainingCredit.add(refundAmount));
                user.setUsedCredit(currentUsedCredit.subtract(refundAmount));
                user.setPendingRepayment(currentPendingRepayment.subtract(totalRepaymentReduction)); // 修正为按比例减少总还款金额
                
                Boolean userUpdateResult = userService.updateById(user);
                if (!userUpdateResult) {
                    logger.error("授信退款，更新用户授信额度失败，orderNo = {}", orderNo);
                    e.setRollbackOnly();
                    return Boolean.FALSE;
                }
                
                // 4. 删除或更新授信支付订单状态
                if (refundAmount.compareTo(creditPaymentOrder.getPaymentAmount()) >= 0) {
                    // 全额退款 - 删除待还款记录
                    creditPaymentOrder.setStatus(3); // 设置为已取消状态
                    creditPaymentOrder.setRemark("订单全额退款，授信额度已恢复");
                } else {
                    // 部分退款 - 按比例更新本金、利息和总还款金额
                    BigDecimal newPaymentAmount = creditPaymentOrder.getPaymentAmount().subtract(refundAmount);
                    BigDecimal newInterestAmount = creditPaymentOrder.getInterestAmount().multiply(refundRatio);
                    BigDecimal newTotalRepaymentAmount = newPaymentAmount.add(newInterestAmount);
                    
                    creditPaymentOrder.setPaymentAmount(newPaymentAmount);
                    creditPaymentOrder.setInterestAmount(newInterestAmount);
                    creditPaymentOrder.setTotalRepaymentAmount(newTotalRepaymentAmount);
                    creditPaymentOrder.setRemainingAmount(newTotalRepaymentAmount);
                    creditPaymentOrder.setRemark("订单部分退款，剩余本金：" + newPaymentAmount + "，剩余利息：" + newInterestAmount);
                }
                
                Boolean orderUpdateResult = creditPaymentOrderService.updateById(creditPaymentOrder);
                if (!orderUpdateResult) {
                    logger.error("授信退款，更新授信支付订单失败，orderNo = {}", orderNo);
                    e.setRollbackOnly();
                    return Boolean.FALSE;
                }
                
                // 5. 生成退款类型的交易记录
                Boolean refundTransactionResult = creditTransactionService.createRefundTransaction(
                    user.getId(), orderNo, refundAmount);
                if (!refundTransactionResult) {
                    logger.error("授信退款，创建退款交易记录失败，orderNo = {}", orderNo);
                    e.setRollbackOnly();
                    return Boolean.FALSE;
                }
            ...
            if (!execute) {
                throw new CrmebException("授信退款处理失败");
            }
            
            return Boolean.TRUE;
        } catch (Exception e) {
            logger.error("授信退款异常，orderNo = {}", orderNo, e);
            throw new CrmebException("授信退款失败：" + e.getMessage());
        }
    }
}
