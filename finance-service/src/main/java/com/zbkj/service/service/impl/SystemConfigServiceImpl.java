...
public class SystemConfigServiceImpl extends ServiceImpl<SystemConfigDao, SystemConfig> implements SystemConfigService {
    ...
    public MyRecord getValuesByKeyList(List<String> keyList) {
        ...
        if (!crmebConfig.getAsyncConfig()) {
            ...
            List<SystemConfig> systemConfigList = dao.selectList(lqw);
            keyList.forEach(k -> {
                // 由于移除了GROUP BY，可能有重复记录，取第一条（最新的，因为按id DESC排序）
                SystemConfig systemConfig = systemConfigList.stream()
                    .filter(config -> config.getName().equals(k))
                    .findFirst()
                    .orElse(null);
    ...
    /**
     * 保存支付配置
     * @param request 支付配置请求
     * @return Boolean
     */
    @Override
    public Boolean savePayConfig(PayConfigRequest request) {
        List<SystemConfigAdminRequest> requestList = new ArrayList<>();
        
        // 微信支付开关
        SystemConfigAdminRequest weixinConfig = new SystemConfigAdminRequest();
        weixinConfig.setName(SysConfigConstants.CONFIG_PAY_WECHAT_OPEN);
        weixinConfig.setValue(request.getPayWeixinOpen());
        requestList.add(weixinConfig);
        
        // 支付宝支付状态
        SystemConfigAdminRequest aliConfig = new SystemConfigAdminRequest();
        aliConfig.setName(SysConfigConstants.CONFIG_ALI_PAY_STATUS);
        aliConfig.setValue(request.getAliPayStatus());
        requestList.add(aliConfig);
        
        // 余额支付状态
        SystemConfigAdminRequest yueConfig = new SystemConfigAdminRequest();
        yueConfig.setName(SysConfigConstants.CONFIG_YUE_PAY_STATUS);
        yueConfig.setValue(request.getYuePayStatus());
        requestList.add(yueConfig);
        
        // 授信支付状态
        SystemConfigAdminRequest creditConfig = new SystemConfigAdminRequest();
        creditConfig.setName(SysConfigConstants.CONFIG_CREDIT_PAY_STATUS);
        creditConfig.setValue(request.getCreditPayStatus());
        requestList.add(creditConfig);
        
        // 微信支付来源
        if (StringUtils.hasText(request.getWechatPaySource())) {
            SystemConfigAdminRequest sourceConfig = new SystemConfigAdminRequest();
            sourceConfig.setName("wechat_pay_source");
            sourceConfig.setValue(request.getWechatPaySource());
            requestList.add(sourceConfig);
        }
        
        return updateByList(requestList);
    }
