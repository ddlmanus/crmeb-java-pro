...
public class GroupBuyActivitySkuServiceImpl extends ServiceImpl<GroupBuyActivitySkuDao, GroupBuyActivitySku> implements GroupBuyActivitySkuService {
    ...
    /**
     * 获取拼团商品列表 这里不是以sku为纬度，是以商品为纬度
     *  1. 先获取当前开启的拼团活动
     *  2. 再根据拼团活动查询对应的拼团商品
     * @param paramRequest 分页参数
     * @return List<GroupBuyActivityFrontResponse>
     */
    @Override
    public List<GroupBuyActivitySkuForSaleListResponse> getGroupBuySkusForSaleList(PageParamRequest paramRequest) {

        List<GroupBuyActivitySkuForSaleListResponse> groupBuyActivityProductResponseList = new ArrayList<>();
        // 获取当前拼团商品列表
        List<GroupBuyActivitySku> groupBuySkusForFrontByList = getGroupBuySkusForFrontByList(paramRequest);

        // 加载当前拼团商品的活动配置
        List<Integer> activityIds = groupBuySkusForFrontByList.stream().distinct().map(GroupBuyActivitySku::getGroupActivityId).collect(Collectors.toList());
        List<GroupBuyActivity> currentGroupBuyList = groupBuyActivityService.listByIds(activityIds);

        for (GroupBuyActivitySku groupBuyActivitySku : groupBuySkusForFrontByList) {

            Product product = productService.getH5Detail(groupBuyActivitySku.getProductId());
            GroupBuyActivitySkuForSaleListResponse skuResponse = new GroupBuyActivitySkuForSaleListResponse();
            skuResponse.setId(groupBuyActivitySku.getId());
            skuResponse.setGroupActivityId(groupBuyActivitySku.getGroupActivityId());
            skuResponse.setProductId(groupBuyActivitySku.getProductId());
            ...
            List<GroupBuyActivity> currentGroupBuys = currentGroupBuyList.stream().filter(item -> item.getId().equals(groupBuyActivitySku.getGroupActivityId())).collect(Collectors.toList());
            if(!currentGroupBuys.isEmpty()){
                skuResponse.setBuyCount(currentGroupBuys.get(0).getBuyCount());
            }

            groupBuyActivityProductResponseList.add(skuResponse);
            skuResponse.setSaleCount(groupBuyRecordService.getOrderDoneCount(groupBuyActivitySku.getGroupActivityId()));
        }

        return groupBuyActivityProductResponseList;
    }
     ...
     * @param paramRequest 分页参数
     */
    private List<GroupBuyActivitySku> getGroupBuySkusForFrontByList(PageParamRequest paramRequest) {
        PageHelper.startPage(paramRequest.getPage(), paramRequest.getLimit());

        LambdaQueryWrapper<GroupBuyActivitySku> queryWrapper = Wrappers.lambdaQuery();
        queryWrapper.eq(GroupBuyActivitySku::getActivityStatus, Boolean.TRUE)
                .eq(GroupBuyActivitySku::getGroupStatus, GroupBuyGroupStatusEnum.GROUP_BUY_ENUM_ACTIVITY_STATUS_PASS.getCode());
        return dao.selectList(queryWrapper);
    }
     ...
     */
    @Override
    public void subSKUStock(Integer groupBuyActivityId, Integer attrValueId, Integer payNum) {
     ...
     */
    @Override
    public void rollBackSKUStock(Order order,Integer groupBuyActivityId, Integer skuId, Integer payNum) {
        LambdaQueryWrapper<GroupBuyActivitySku> currentSku = Wrappers.lambdaQuery();
        currentSku.eq(GroupBuyActivitySku::getGroupActivityId, groupBuyActivityId)
   ...
   public void deleteSkusIdsForProductTurnOff(List<Integer> finalSkuIdList){
       ...
       List<Integer> ids = skus.stream().map(GroupBuyActivitySku::getSkuId).collect(Collectors.toList());
