...
public class ProductServiceImpl extends ServiceImpl<ProductDao, Product>
    ...
    public Boolean save(ProductAddRequest request) {
        // 商品信息校验
        saveProductValidator(request.getType(), request.getSpecType(), request.getAttrValue(),
        ...
        if (request.getType().equals(ProductConstants.PRODUCT_TYPE_CDKEY)) {
            List<Integer> cdkIdList = request.getAttrValue().stream().map(ProductAttrValueAddRequest::getCdkeyId).distinct().collect(Collectors.toList());
        ...
        if (StrUtil.isBlank(request.getKeyword())) {
            request.setKeyword("");
        } else if (request.getKeyword().length() > 255) {
        ...
        List<ProductAttr> attrList = addRequestList.stream().map(e -> {
            ...
            attr.setId(null);
            attr.setType(product.getType());
            attr.setMarketingType(product.getMarketingType());
            return attr;
        }).collect(Collectors.toList());
     ...
     * 商品sku
     *
     * @param attrValue json字符串或对象
     * @return sku
     */
    private String getSku(String attrValue) {
        try {
            // 如果attrValue为空或null，返回默认值
            if (StrUtil.isBlank(attrValue)) {
                return "默认";
            }
            
            // 检查是否为JSON对象字符串格式
            String trimmedValue = attrValue.trim();
            if (trimmedValue.startsWith("{") && trimmedValue.endsWith("}")) {
                // 解析JSON对象，处理各种类型的值
                JSONObject jsonObject = JSONObject.parseObject(attrValue, Feature.OrderedField);
                ...
                // 如果不是标准JSON格式，直接返回原值作为SKU
                LOGGER.warn("SKU属性值不是标准JSON格式，使用原值：" + attrValue);
                return attrValue;
            }
        } catch (Exception e) {
            LOGGER.error("解析SKU属性值失败，attrValue: " + attrValue + ", 错误: " + e.getMessage(), e);
            // 解析失败时返回默认值
            return "默认";
        }
    }
    ...
    public Boolean update(ProductAddRequest productRequest, SystemAdmin admin) {
        ...
        saveProductValidator(productRequest.getType(), productRequest.getSpecType(), productRequest.getAttrValue(),
        ...
        if (tempProduct.getType().equals(ProductConstants.PRODUCT_TYPE_CDKEY)) {
            List<Integer> cdkIdList = productRequest.getAttrValue().stream().map(ProductAttrValueAddRequest::getCdkeyId).distinct().collect(Collectors.toList());
            List<CdkeyLibrary> cdkeyLibraryList = cdkeyLibraryService.findByIdList(cdkIdList);
            for (CdkeyLibrary cdkeyLibrary : cdkeyLibraryList) {
                if (!admin.getMerId().equals(cdkeyLibrary.getMerId())) {
                    throw new CrmebException(ProductResultCode.PRODUCT_CDKEY_LIBRARY_NOT_EXIST);
        ...
        List<ProductAttrValue> tempAttrValueList = productAttrValueService.getListByProductIdAndType(tempProduct.getId(), tempProduct.getType(), tempProduct.getMarketingType());
        ...
        Boolean execute = transactionTemplate.execute(e -> {
            ...
            // 先删除原用attr+value
            attrService.deleteByProductIdAndType(product.getId(), product.getType(), product.getMarketingType());
        ...
        if (execute) {
            ...
            if (!merchant.getProductSwitch() && productRequest.getIsAutoUp()
                ...
                List<ProductAttrValue> skuList = productAttrValueService.getListByProductIdAndType(product.getId(), product.getType(), product.getMarketingType());
    ...
    public ProductInfoResponse getInfo(Integer id, SystemAdmin admin) {
        ...
        List<ProductAttr> attrList = attrService.getListByProductIdAndType(product.getId(), product.getType(), product.getMarketingType());
        productInfoResponse.setAttr(attrList);

        List<ProductAttrValue> attrValueList = productAttrValueService.getListByProductIdAndType(product.getId(), product.getType(), product.getMarketingType());
    ...
    public Boolean putOnShelf(Integer id, SystemAdmin admin) {
        ...
        // 获取商品skuid
        List<ProductAttrValue> skuList = productAttrValueService.getListByProductIdAndType(id, product.getType());
        List<Integer> skuIdList = skuList.stream().map(ProductAttrValue::getId).collect(Collectors.toList());
        return transactionTemplate.execute(e -> {
            dao.updateById(product);
            if (CollUtil.isNotEmpty(skuIdList)) {
    ...
    public PageInfo<Product> getIndexProduct(Integer cid, PageParamRequest pageParamRequest) {
        ...
        lqw.select(Product::getId, Product::getMerId, Product::getImage, Product::getName, Product::getUnitName,
                ...
                Product::getBrandId, Product::getIsPaidMember, Product::getVipPrice);
        getForSaleWhere(lqw);
    ...
    public PageInfo<ProductFrontResponse> findH5List(ProductFrontSearchRequest request, PageParamRequest pageRequest) {
        ...
        List<ProductFrontResponse> reslutList = dao.findH5List(map);
        List<ProductFrontResponse> responseList = new ArrayList<>();
        if (!CollUtil.isEmpty(reslutList)) {
            List<Integer> proIdList = reslutList.stream().map(ProductFrontResponse::getId).collect(Collectors.toList());
            List<ProductAttrValue> attrValueList = productAttrValueService.getByProductIdList(proIdList);
            List<ProductAttr> productAttrs = attrService.getByProductIds(proIdList);
            Map<Integer, List<ProductAttrValue>> attrValueMap = attrValueList.stream().collect(Collectors.groupingBy(ProductAttrValue::getProductId));
            Map<Integer, List<ProductAttr>> attrMap = productAttrs.stream().collect(Collectors.groupingBy(ProductAttr::getProductId));
            reslutList.stream().forEach(productItem -> {
                ...
                if(CollUtil.isNotEmpty(attrValueMap)){
                    List<ProductAttrValue> productAttrValueList = attrValueMap.get(productItem.getId());
                    if(CollUtil.isNotEmpty(productAttrValueList)){
                        //对排序字段新增非空判断 防止排序异常  排序字段为空时  排序会出现异常
                        ...
                        productAttrValueList = productAttrValueList.stream().filter(productAttrValue -> Objects.nonNull(productAttrValue.getPrice())).sorted(Comparator.comparing(ProductAttrValue::getPrice)).collect(Collectors.toList());
                        if(CollUtil.isNotEmpty(productAttrValueList)&&Objects.nonNull(productAttrValueList.get(0).getPrice())){
                            response.setPrice(productAttrValueList.get(0).getPrice());
                        }
                    }
                }
                if(CollUtil.isNotEmpty(attrMap)){
                    response.setProductAttr(attrMap.get(productItem.getId()));
                }
                response.setProductValue(skuMap);
                responseList.add(response);
            });
            PageInfo<ProductFrontResponse> pageInfo = CommonPage.copyPageInfo(page, responseList);
            return pageInfo;
        }
    ...
    public PageInfo<PlatformProductListResponse> getPlatformPageList(PlatProductSearchRequest request) {
        ...
        if (ObjectUtil.isNotNull(request.getCategoryId())) {
            ...
            if (ObjectUtil.isNull(category)) {
                // 分类不存在，记录日志并忽略此筛选条件
                LOGGER.warn("商品分类查询失败，分类ID不存在：{}", request.getCategoryId());
            } else if (category.getLevel().equals(3)) {
                // 三级分类，直接使用
                map.put("categoryIds", request.getCategoryId());
                LOGGER.info("使用三级分类筛选商品，分类ID：{}", request.getCategoryId());
            } else {
                // 非三级分类，查找所有子分类中的三级分类
                LOGGER.info("检测到非三级分类，分类ID：{}，级别：{}，开始查找子分类", request.getCategoryId(), category.getLevel());
                ...
                if (CollUtil.isEmpty(cateIdList)) {
                    // 没有找到三级子分类，直接使用当前分类ID进行筛选
                    LOGGER.info("在分类ID：{} 下未找到三级子分类，直接使用当前分类进行筛选", request.getCategoryId());
                    map.put("categoryIds", request.getCategoryId());
                } else {
                    String categoryIds = String.join(",", cateIdList);
                    map.put("categoryIds", categoryIds);
                    LOGGER.info("使用三级子分类筛选商品，子分类IDs：{}", categoryIds);
                }
            }
        }
...
//        lambdaQueryWrapper.eq(Product::getIsShow, Boolean.TRUE);
    ...
    public Boolean audit(ProductAuditRequest request) {
        ...
        return transactionTemplate.execute(e -> {
            if (merchant.getIsSwitch() && product.getIsAutoUp()) {
                product.setIsShow(true);
                List<ProductAttrValue> skuList = productAttrValueService.getListByProductIdAndType(product.getId(), product.getType());
                List<Integer> skuIdList = skuList.stream().map(ProductAttrValue::getId).collect(Collectors.toList());
                if (CollUtil.isNotEmpty(skuIdList)) {
                    cartService.productStatusNoEnable(skuIdList);
                }
    ...
    public PageInfo<Product> findMerchantProH5List(MerchantProductSearchRequest request, PageParamRequest pageParamRequest) {
        ...
        if (ObjectUtil.isNotNull(request.getCid()) && request.getCid() > 0) {
            lqw.apply(" find_in_set({0}, cate_id)", request.getCid());
        }
        if (ObjectUtil.isNotNull(request.getMaxPrice())) {
    ...
    public List<ProductTabsHeaderResponse> getPlatformTabsHeader(PlatProductTabsHeaderRequest request) {
        ...
        if (ObjectUtil.isNotNull(request.getCategoryId())) {
            ...
            if (ObjectUtil.isNull(category)) {
                ...
                // 三级分类，直接使用
                map.put("categoryIds", request.getCategoryId());
                LOGGER.info("使用三级分类筛选表头统计，分类ID：{}", request.getCategoryId());
            } else {
                // 非三级分类，查找所有子分类中的三级分类
                LOGGER.info("检测到非三级分类，分类ID：{}，级别：{}，开始查找子分类进行表头统计", request.getCategoryId(), category.getLevel());
                List<ProductCategory> categoryList = productCategoryService.findAllChildListByPid(category.getId(), category.getLevel());
                List<String> cateIdList = categoryList.stream().filter(e -> e.getLevel().equals(3)).map(e -> e.getId().toString()).collect(Collectors.toList());
                                 if (CollUtil.isEmpty(cateIdList)) {
                     // 没有找到三级子分类，直接使用当前分类ID进行筛选
                     LOGGER.info("在分类ID：{} 下未找到三级子分类，直接使用当前分类进行表头统计", request.getCategoryId());
                     map.put("categoryIds", request.getCategoryId());
                 } else {
                     String categoryIds = String.join(",", cateIdList);
                     map.put("categoryIds", categoryIds);
                     LOGGER.info("使用三级子分类筛选表头统计，子分类IDs：{}", categoryIds);
                 }
            }
        }
        if (ObjectUtil.isNotNull(request.getMerId())) {
    ...
    public Boolean reviewFreeEdit(ProductReviewFreeEditRequest request, SystemAdmin admin) {
        ...
        List<ProductAttrValue> attrValueList = productAttrValueService.getListByProductIdAndType(product.getId(), product.getType());
     ...
     */
    @Override
    public MyRecord copyProduct(String url, SystemAdmin currentMerchantAdmin) {
        // 校验当前商户的copy余量
     ...
     */
    @Override
    public PageInfo<ProductActivityResponse> getActivitySearchPage(ProductActivitySearchRequest request, PageParamRequest pageRequest, SystemAdmin admin) {
        Page<Product> page = PageHelper.startPage(pageRequest.getPage(), pageRequest.getLimit());
        Map<String, Object> map = new HashMap<>();
    ...
    public PageInfo<ProductMarketingResponse> getMarketingSearchPage(PlatProductMarketingSearchRequest request, SystemAdmin admin) {
        ...
        responseList.forEach(response -> {
            List<ProductAttrValue> attrValueList = productAttrValueService.getListByProductIdAndType(response.getId(),
                    response.getType(), ProductConstants.PRODUCT_MARKETING_TYPE_BASE);
            response.setAttrValue(attrValueList);
        });
        return CommonPage.copyPageInfo(page, responseList);
    }
...
//        }
        ...
        responseList.forEach(response -> {
            List<ProductAttrValue> attrValueList = productAttrValueService.getListByProductIdAndType(response.getId(), response.getType(), response.getMarketingType());
            response.setAttrValue(attrValueList);
        });
        return CommonPage.copyPageInfo(page, responseList);
    }
...
//            }
     ...
     */
    @Override
    public Boolean batchSubmitAudit(List<Integer> idList, SystemAdmin admin) {
        List<Product> productList = findByIds(idList);
        if (CollUtil.isEmpty(productList) || productList.size() != idList.size()) {
            throw new CrmebException(CommonResultCode.VALIDATE_FAILED, "商品数量不符或商品未找到");
        }
    ...
    public Boolean saveIntegralProduct(IntegralProductAddRequest request) {
        ...
        List<IntegralProductAttrValueAddRequest> attrValueAddRequestList = request.getAttrValue();
        //计算价格
     ...
     */
    @Override
    public Product getBySanmuGoodsId(String sanmuGoodsId) {
        if (StrUtil.isBlank(sanmuGoodsId)) {
            return null;
        }
        LambdaQueryWrapper<Product> lqw = new LambdaQueryWrapper<>();
        lqw.eq(Product::getSanmuGoodsId, sanmuGoodsId);
        lqw.eq(Product::getIsDel, false);
        return dao.selectOne(lqw);
    }
     ...
     */
    @Override
    public Boolean updateForSync(ProductAddRequest request) {
        // 根据三牧优选商品ID查找现有商品
        Product existingProduct = getBySanmuGoodsId(request.getSanmuGoodsId());
        if (existingProduct == null) {
            throw new CrmebException("商品不存在，无法更新");
        }
        ...
        // 保持原有的一些字段不变
        product.setMerId(existingProduct.getMerId());
        product.setCreateTime(existingProduct.getCreateTime());
        product.setIsShow(existingProduct.getIsShow());
        product.setAuditStatus(existingProduct.getAuditStatus());
        ...
        return transactionTemplate.execute(e -> {
            try {
                ...
                // 添加新的属性和属性值
                if (CollUtil.isNotEmpty(request.getAttr())) {
                    List<ProductAttr> attrs = new ArrayList<>();
                    for (ProductAttrAddRequest attrAddRequest : request.getAttr()) {
                        ProductAttr attr = new ProductAttr();
                        BeanUtils.copyProperties(attrAddRequest, attr);
                        attr.setProductId(product.getId());
                        attr.setType(product.getType());
                        attr.setMarketingType(product.getMarketingType());
                        attrs.add(attr);
                    }
                    attrService.saveBatch(attrs);
                }

                if (CollUtil.isNotEmpty(request.getAttrValue())) {
                    List<ProductAttrValue> attrValues = new ArrayList<>();
                    for (ProductAttrValueAddRequest valueAddRequest : request.getAttrValue()) {
                        ProductAttrValue attrValue = new ProductAttrValue();
                        BeanUtils.copyProperties(valueAddRequest, attrValue);
                        attrValue.setProductId(product.getId());
                        attrValue.setType(product.getType());
                        attrValue.setMarketingType(product.getMarketingType());
                        attrValue.setSku(getSku(valueAddRequest.getAttrValue()));
                ...
                if (StrUtil.isNotBlank(request.getContent())) {
                    ProductDescription description = productDescriptionService.getByProductIdAndType(product.getId(), product.getType(), product.getMarketingType());
                    if (description != null) {
                        description.setDescription(request.getContent());
                        productDescriptionService.updateById(description);
                    } else {
                        description = new ProductDescription();
                        description.setProductId(product.getId());
                        description.setDescription(request.getContent());
                        description.setType(product.getType());
                        description.setMarketingType(product.getMarketingType());
                ...
                e.setRollbackOnly();
                LOGGER.error("更新商品失败：{}", ex.getMessage(), ex);
                return false;
            }
        });
    }
