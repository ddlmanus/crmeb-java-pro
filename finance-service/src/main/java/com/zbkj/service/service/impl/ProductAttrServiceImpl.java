...
public class ProductAttrServiceImpl extends ServiceImpl<ProductAttrDao, ProductAttr>
    ...
    @Override
    public ProductAttr getByName(String unitName) {
        LambdaQueryWrapper<ProductAttr> lqw = Wrappers.lambdaQuery();
        lqw.eq(ProductAttr::getAttrName, unitName);
        return dao.selectOne(lqw);
    }

    @Override
    public List<ProductAttr> getByProductIds(List<Integer> proIdList) {
        LambdaQueryWrapper<ProductAttr> lqw = Wrappers.lambdaQuery();
        lqw.in(ProductAttr::getProductId, proIdList);
        return dao.selectList(lqw);
    }
}
