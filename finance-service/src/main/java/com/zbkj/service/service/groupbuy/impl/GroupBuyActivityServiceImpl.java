...
//            groupBuyActivityResponse.setGroupBuyActivitySkuResponses(groupBuyActivitySkuService.getListByGroupActivityId(groupBuyActivity.getId()));
        ...
        for (Map.Entry<Integer, List<GroupBuyActivitySku>> productIdEntry : skuListGroupByProductIdMap.entrySet()) {
            ...
            for (GroupBuyActivitySku groupBuyActivitySku : groupBuyActivitySkuListByProductId) {
                ...
                List<AttrValueResponse> seamSkuList =
                        productInfoResponse.getAttrValue().stream().filter(attrValue -> attrValue.getId().equals(groupBuyActivitySku.getSkuId())).collect(Collectors.toList());
    ...
    private List<GroupBuyActivity> getListForShopActive(Integer limit) {
        ...
        if(ObjectUtil.isEmpty(limit)){
            throw new CrmebException("拼团限制不能为空");
        }
    ...
    public GroupBuyActivityFrontResponse getGroupBuyActivityFrontIndex(Integer limit) {
        ...
        for (GroupBuyActivity groupBuyActivity : groupBuyActivityList) {
            ...
            List<GroupBuyActivitySku> skus = groupBuyActivitySkuService.getListByGroupActivityId(groupBuyActivity.getId());
            // 主商品的价格数据拿第一个sku的数据
            ...
            if(ObjectUtil.isNotEmpty(skus)){
                ...
                BeanUtils.copyProperties(currentActivitySku, skuResponse);

                Product product = productService.getById(currentActivitySku.getProductId());
                // 排除审核失败的商品 以及删除标记的商品
                if(product.getIsDel() || product.getAuditStatus().equals(ProductConstants.AUDIT_STATUS_WAIT) ||
                        product.getAuditStatus().equals(ProductConstants.AUDIT_STATUS_FAIL)) continue;
                ProductAttrValue sku = productAttrValueService.getById(currentActivitySku.getSkuId());
                AttrValueResponse attrValue = new AttrValueResponse();

                BeanUtils.copyProperties(sku, attrValue);
                ...
                attrValueResponseList.add(attrValue);
                skuResponse.setAttrValue(attrValueResponseList);

                skuResponse.setLatestBuyCount(groupBuyUserService.getOrderDoneCountByProductIdAndActivityId(currentActivitySku.getProductId(), groupBuyActivity.getId()));

                productResponse.setProductId(currentActivitySku.getProductId());
