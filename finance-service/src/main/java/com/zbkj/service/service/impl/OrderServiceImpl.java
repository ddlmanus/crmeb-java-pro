...
//                    lqw.eq(Order::getGroupBuyRecordStatus, 0);
        ...
        if(Objects.nonNull(user.getFarmType())&&user.getFarmType().equals(FarmTypeEnum.TENANT_TYPE_COOPERATIVE.getCode())){
            //查询该机构下的所有用户
            List<Integer> userIds = userService.getUserIdsByOrganizationId(user.getOrganizationId());
...
//        }
     ...
     */
    @Override
    public Boolean verificationOrderByCode(String verifyCode, SystemAdmin systemAdmin) {
        ...
        if (ObjectUtil.isNull(merchantOrder)) {
            throw new CrmebException(CommonResultCode.VALIDATE_FAILED, "请选择正确的核销码");
        }
...
//        temMap.put("thing7", "您的订单已发货");
    ...
    /**
     * 获取指定日期的活跃商户数量（有订单的商户）
     *
     * @param date 日期，yyyy-MM-dd格式
     * @return Integer
     */
    @Override
    public Integer getActiveMerchantCountByDate(String date) {
        QueryWrapper<Order> wrapper = new QueryWrapper<>();
        wrapper.select("DISTINCT mer_id");
        wrapper.eq("is_del", false);
        wrapper.apply("date_format(create_time, '%Y-%m-%d') = {0}", date);
        List<Order> orderList = dao.selectList(wrapper);
     ...
     */
    @Override
    public Integer getPaidOrderNumByDate(String date) {
        QueryWrapper<Order> wrapper = new QueryWrapper<>();
        wrapper.select("id");
    ...
    public BigDecimal getPendingSettlementAmount() {
        ...
        wrapper.select("pay_price");
        wrapper.eq("paid", true);
        wrapper.eq("status", OrderConstants.ORDER_STATUS_COMPLETE);
        wrapper.eq("is_del", false);
        // 这里可以添加结算状态的判断，暂时查询已完成但未结算的订单
        List<Order> orderList = dao.selectList(wrapper);
        if (CollUtil.isEmpty(orderList)) {
            return BigDecimal.ZERO;
        }
    ...
    public BigDecimal getSettledAmount() {
        // 这里需要根据实际的结算表或结算状态来查询
        // 暂时返回0，实际项目中需要根据业务逻辑实现
        return BigDecimal.ZERO;
    }
