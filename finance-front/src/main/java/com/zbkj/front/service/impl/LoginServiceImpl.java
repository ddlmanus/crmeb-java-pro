...
import com.zbkj.common.enums.FarmInstitutionEnum;
import com.zbkj.common.enums.TenantTypeEnum;
...
//                throw new CrmebException("小程序获取手机号加密数据不能为空");
    ...
    private Integer createMerchantApply(User user, ThirdLogin thirdLogin) {
        ...
        merchantApply.setAuditStatus(com.zbkj.common.constants.MerchantConstants.AUDIT_STATUS_WAIT); // 待审核
    ...
    private void autoAuditMerchantApply(Integer merchantApplyId) {
        try {
            ...
            // 如果已经审核过，直接返回
            if (!merchantApply.getAuditStatus().equals(com.zbkj.common.constants.MerchantConstants.AUDIT_STATUS_WAIT)) {
                log.info("商户申请已审核，申请ID: {}, 状态: {}", merchantApplyId, merchantApply.getAuditStatus());
                return;
            }
            
            // 更新申请状态为审核通过
            merchantApply.setAuditStatus(com.zbkj.common.constants.MerchantConstants.AUDIT_STATUS_SUCCESS);
            ...
            org.springframework.beans.BeanUtils.copyProperties(merchantApply, merchantAddRequest);
