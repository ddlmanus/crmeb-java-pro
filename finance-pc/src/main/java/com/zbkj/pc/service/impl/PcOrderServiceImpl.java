...
public class PcOrderServiceImpl implements PcOrderService {
    ...
    private final Logger logger = LoggerFactory.getLogger(PcOrderServiceImpl.class);
    ...
    @Autowired
    private PcSeckillService pcSeckillService;
    @Autowired
    ...
    private OrderNoResponse createPreOrderResponse(Integer userId, PreOrderInfoVo preOrderInfoVo) {
        ...
        if(preOrderInfoVo.getType().equals(OrderConstants.ORDER_TYPE_PITUAN)){
            preOrderInfoVo.getMerchantOrderVoList().stream().map(item -> item.getOrderInfoList().stream().map(ov -> {
    ...
    public OrderNoResponse createOrder(CreateOrderRequest orderRequest) {
        ...
        if (orderInfoVo.getType().equals(OrderConstants.ORDER_TYPE_SECKILL)) {
            return pcSeckillService.createOrder(orderRequest, orderInfoVo, user);
        }
        // 拼团商品单独处理
        ...
        if(Objects.nonNull(user.getUserType())){
           if(Objects.nonNull(user.getFarmType())&&user.getFarmType().equals(FarmTypeEnum.TENANT_TYPE_COOPERATIVE.getCode())){
               order.setOrganizationId(user.getOrganizationId());
           }
        }
        // 商户订单
    ...
    private List<MyRecord> validateProductStock(PreOrderInfoVo orderInfoVo) {
        ...
        if (orderInfoVo.getType().equals(OrderConstants.ORDER_TYPE_SECKILL)) {// 秒杀订单
            recordList.add(pcSeckillService.validateCreateOrderProductStock(orderInfoVo));
            return recordList;
        }
     ...
     */
    @Override
    public void getFreightFee(PreOrderInfoVo orderInfoVo, UserAddress userAddress, Boolean userIsPaidMember) {
                ...
                switch (shippingTemplate.getType()) {
                    case ShippingTemplatesConstants.CHARGE_MODE_TYPE_NUMBER: // 件数
                        ...
                        if (num <= shippingTemplatesRegion.getFirst().intValue()) {
                            ...
                            // 更新运费计算公式 2024-4-15
                            BigDecimal renewalPrice = shippingTemplatesRegion.getRenewalPrice().multiply(new BigDecimal(String.valueOf(renewalNum))).divide(shippingTemplatesRegion.getRenewal(), 2, RoundingMode.HALF_UP);
                        ...
                        if (detailVoList.size() == 1) {
                            ...
                            for (int i = 0; i < detailVoList.size(); i++) {
                                ...
                                // 更新运费计算公式 2024-4-15
                                BigDecimal multiply = postageFee.multiply(new BigDecimal(detail.getPayNum().toString())).divide(new BigDecimal(tempNum.toString()), 2, RoundingMode.HALF_UP);
    ...
    private PreOrderInfoVo validatePreOrderRequest_V1_7(PreOrderRequest request, User user) {
        ...
        switch (request.getPreOrderType()) {
            ...
            case OrderConstants.PLACE_ORDER_TYPE_SECKILL:
                // 秒杀只支持单商品支付
                PreMerchantOrderVo preMerchantSeckillOrderVo = pcSeckillService.validatePreOrderSeckill(request.getOrderDetails().get(0));
