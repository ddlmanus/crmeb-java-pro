 ...
 */
@Service
public class PcSeckillServiceImpl implements PcSeckillService {

    private final Logger logger = LoggerFactory.getLogger(PcSeckillServiceImpl.class);
    ...
    public ProductDetailResponse getProductDetail(Integer id) {
        ...
        List<ProductAttr> productAttrList = productAttrService.getListByProductIdAndType(seckillProduct.getId(), seckillProduct.getType(), ProductConstants.PRODUCT_MARKETING_TYPE_SECKILL);
        List<ProductAttrValue> productAttrValueList = productAttrValueService.getListByProductIdAndType(seckillProduct.getId(), seckillProduct.getType(), ProductConstants.PRODUCT_MARKETING_TYPE_SECKILL);
    ...
    public OrderNoResponse createOrder(CreateOrderRequest orderRequest, PreOrderInfoVo orderInfoVo, User user) {
        ...
        // 计算订单各种价格
        pcOrderService.getFreightFee(orderInfoVo, userAddress, false);
