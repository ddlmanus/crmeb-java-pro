...
public class SeckillServiceImpl implements SeckillService {
    ...
    public Boolean createActivity(SeckillActivitySaveRequest request) {
        ...
        if (CollUtil.isNotEmpty(request.getProductList())) {
            request.getProductList().forEach(productRequest -> {
                ...
                List<ProductAttr> productAttrList = productAttrService.getListByProductIdAndType(product.getId(), product.getType());
                if (CollUtil.isEmpty(productAttrList)) {
                    throw new CrmebException(CommonResultCode.VALIDATE_FAILED, "没有对应的商品规格信息，商品ID = " + productRequest.getId());
                }
                List<ProductAttrValue> productAttrValueList = productAttrValueService.getListByProductIdAndType(product.getId(), product.getType());
            ...
            if (CollUtil.isNotEmpty(productList)) {
                productList.forEach(product -> {
                    ...
                    attrs.forEach(a -> {
                        ...
                        a.setMarketingType(ProductConstants.PRODUCT_MARKETING_TYPE_SECKILL);
                    });
    ...
    public SeckillActivityDetailResponse activityDetail(Integer id) {
        ...
        if (CollUtil.isNotEmpty(productList)) {
            ...
            productList.forEach(p -> {
                p.setAttrValue(productAttrValueService.getListByProductIdAndType(p.getId(), ProductConstants.PRODUCT_MARKETING_TYPE_SECKILL));
    ...
    public Boolean merchantDeleteProduct(SeckillProductBatchRequest request) {
        ...
        Boolean execute = transactionTemplate.execute(e -> {
            seckillProductService.delete(request.getIds());
            productAttrService.batchDeleteByProductIdAndMarketingType(proIdList, ProductConstants.PRODUCT_MARKETING_TYPE_SECKILL);
        ...
        Boolean execute = transactionTemplate.execute(e -> {
            try {
                productList.forEach(product -> {
                    ...
                    result = productAttrService.saveBatch(attrs);
                    if (!result) {
                        throw new CrmebException("生成秒杀商品规格失败");
                    }
                    List<ProductAttrValue> attrValues = attrValueList.stream().filter(av -> av.getProductId().equals(pid)).collect(Collectors.toList());
                    attrValues.forEach(av -> {
                        av.setId(null);
