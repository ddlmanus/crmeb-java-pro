...
public class MerchantProductController {
    ...
    @LogControllerAnnotation(intoDB = true, methodType = MethodType.ADD, description = "同步商品")
    // @PreAuthorize("hasAuthority('merchant:product:sync')")  // 临时注释，方便测试
    @ApiOperation(value = "同步商品")
    @RequestMapping(value = "/sync", method = RequestMethod.POST)
    public CommonResult<ProductSyncResponse> syncProducts(@RequestBody @Validated ProductSyncRequest request) {
        ProductSyncResponse response = productService.syncProducts(request);
