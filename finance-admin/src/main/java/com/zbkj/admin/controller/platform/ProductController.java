...
public class ProductController {
    ...
    @PreAuthorize("hasAuthority('platform:product:sync')")
    @ApiOperation(value = "同步CRMEB官网商品数据")
    @RequestMapping(value = "/sync", method = RequestMethod.POST)
    @LogControllerAnnotation(intoDB = true, methodType = MethodType.ADD, description = "同步CRMEB官网商品数据")
    public CommonResult<Boolean> syncProducts() {
        return CommonResult.success(productService.syncProductsFromCrmeb());
    }

    @ApiOperation(value = "同步CRMEB官网商品数据")
    @PostMapping("/sync-crmeb-products")
    public CommonResult<Boolean> syncCrmebProducts() {
        return CommonResult.success(productService.syncProductsFromCrmeb());
    }
}
