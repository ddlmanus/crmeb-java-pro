...
//    private RedisTemplate<String, Object> redisTemplate;
    ...
    public <T> T get(String key) {
        ...
        long delayMillis = 1000; // 每次重试之间的延迟
        ...
        for (int i = 0; i < retryCount; i++) {
            try {
                ...
                if (i < retryCount - 1) {
                    try {
                        Thread.sleep(delayMillis); // 等待一段时间后再次尝试
                    ...
                    // 记录异常信息或采取其他措施
                    System.err.println("Failed to get value from Redis after " + retryCount + " attempts: " + e.getMessage());
                }
            }
        }
        return null; // 如果所有尝试都失败，则返回null
    }
